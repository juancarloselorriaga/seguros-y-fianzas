<template>
<v-card class="py-5">
  <!-- <v-container fill-height> -->
    <v-layout column class="white border-radius">
      <v-layout justify-center class="mt-4 mb-2">
        <v-flex xs8>
          <v-text-field
          v-model="search"
          append-icon="search"
          label="Buscar"
          single-line
          color="red lighten-1"
          hint="Busca por nombre de cliente"
          clearable
        ></v-text-field>
        </v-flex>
      </v-layout>
      <v-data-table
        :headers="headers"
        :items="clientes"
        :search="search"
        rows-per-page-text="Elementos por página"
        no-data-text="No hay datos disponibles"
        :rows-per-page-items="pagination.rowsPerPageItems"
        :pagination.sync="pagination"
      >
        <template v-slot:items="props">
          <td class="text-xs-left blue-grey--text text--darken-4 text-capitalize subheading">
            <v-layout justify-start>{{ props.item.name }}</v-layout>
          </td>
          <td class="text-xs-center blue-grey--text text--darken-4 text-capitalize">
            <v-layout justify-center>
              <v-avatar size="32" color="green">
              <span class="white--text title font-weight-bold">{{ props.item.vigentes }}</span>
            </v-avatar>
            </v-layout>
          </td>
          <td class="justify-center layout px-0">
            <v-icon>visibility</v-icon>
          </td>
        </template>
        <template v-slot:no-results>
          <v-alert
            :value="true"
            color="error"
            icon="warning"
          >La búsqueda de "{{ search }}" no coincide con ningún evento.</v-alert>
        </template>
      </v-data-table>
    </v-layout>
  <!-- </v-container> -->
</v-card>
</template>

<script>
export default {
  name: 'ListadoClientes',
  data() {
    return {
      search: "",
      pagination: {
        rowsPerPage: 5, //default filas al cargar la página
        sortBy: "nombre",
        descending: true
      },
      headers: [
        { text: "Nombre", align: "left", value: "name" },
        { text: "Pólizas vigentes", align: "center", value: "vigentes" },
        { text: "Acciones", align: "center", value: "icon" },
      ],
      clientes: [{name: 'Juan Carlos Chávez Elorriaga', vigentes: '4'}]
    };
  }
};
</script>

<style scoped>

.v-sheet {
  border-radius: 50px;
}

</style>

